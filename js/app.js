...
var cache_dinamico = function(data_json){
    ...
    caches.delete("Pe_na_areia_dinamico").then(function(){
        ...
        caches.open("Pe_na_areia_dinamico").then(function(cache){
...
template_produto = function(id, img, nome, descricao, preco, url){
    return '<div class="col-12 col-md-4">'+
        '<div class="card">'+
            ...
            '<strong>R$ '+preco+'</strong>&nbsp;<input type="number" min="1" max="99" value="1" id="quat_'+id+'" class="input_quant">&nbsp;<button onClick="javascript:addProduto('+id+',document.getElementById(\'quat_'+id+'\').value);" class="btn btn-primary btAddCarrinho">Adicionar ao Carrinho</button>'+
            '</div>'+
        '</div>'+
    '</div>';
}
